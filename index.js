...
async function processRace(raceUrl){             
    ...
    $('div.raceMeeting_select').each((i, element) => {
        const date = $(element).find('span.f_fl.f_fs13').text().replace("Race Meeting:","").trim().split(/\s+/,1); 
        const venueLocation = $(element).find('span.f_fl.f_fs13').text().replace("Race Meeting:","").trim().split(/\s\s+/);
    ...
    $('div.race_tab').each((i, element) => {
        ...
        const distance = $(element).find('tr').slice(2,3).text().replace("M","").trim().split(/\s\s+|-/);
        ...
        hkjcRacingResultsData["raceIndex"] = raceIndex[2];
...
async function main(){
    const url = "https://racing.hkjc.com/racing/information/English/Racing/LocalResults.aspx";  //The HKJC latest local race results link
    ...
    fs.writeFile('hkjcResult.json', JSON.stringify(result),(error)=>{          //Save the JSON file
          if(error){
  ...
  (async() => {                                                             // Save the Excel XLSX file 
    ...
    for(const data of allData){
        ...
        const races = Object.values(data);
          ...
          XLSX.writeFileAsync("hkjcResult.xlsx", workbook, { compression: true }, (error)=>{
            if(error){
